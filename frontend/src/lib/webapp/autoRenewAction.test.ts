...
type TestOverrides = { busy?: boolean; deps?: Record<string, unknown> };

function makeAction(overrides: TestOverrides = {}) {
  ...
  return {
    action: createAutoRenewAction(deps as unknown as Parameters<typeof createAutoRenewAction>[0]),
    busyUpdates,
    deps,
  };
}
